...
const Documents: React.FC<DocumentsProps> = ({ mainData }) => {
    const [visibleCount, setVisibleCount] = useState(5);
    const [isMobile, setIsMobile] = useState(false);
    const allowButtons = mainData.documents && mainData.documents.length > 1;
    ...
    const visibleDocuments = mainData.documents ? mainData.documents.slice(0, visibleCount) : [];
    ...
    return (
        <BorderCard className={styles.documents}>
            <h2>Документы:</h2>
            <div className={styles.documentsDownload}>
                {mainData?.documents !== null && mainData?.documents !== undefined && (
                    <>
                        {(isMobile ? mainData.documents : visibleDocuments).map((doc, index) => (
                                    ...
                                    <button
                                        onClick={() => {
                                            if (doc?.url?.url) {
                                                ...
                                                dummyLink.download = doc.name || "document";
            ...
            <div className={styles.documentsButtons}>
                ...
                {allowButtons && !isMobile && mainData.documents && visibleCount < mainData.documents.length && (
                    <div className={styles.seeMore} onClick={handleShowMore}>
                        <span>Посмотреть все</span>
                    </div>
                )}
                {allowButtons && !isMobile && visibleCount > 5 && (
                    <div className={styles.hideAll} onClick={handleHideAll}>
